...
function Navbar() {
  ...
  const MobileNavItem = () => <div className={styles.mNavItem}>
    ...
    {toggle&&<section className={styles.mPortal}><div className={styles.hamMenu}>
      <div className={styles.themeIcon}> <ThemeMode
      onClick={() => setDarkMode((prevMode) => !prevMode)}
      isDark={isDarkMode}
    /></div><div className={styles.closeHam} onClick={()=>setToggle(!toggle)}></div>
        ...
        {navList.map((navItem) => {
          return (
            <span
              key={navItem.id}
              className={
                router.pathname === navItem.path ? styles.mactive : undefined
              }>
              <Link href={navItem.path}>{navItem.title}</Link>
            </span>
          );
